 ...
 *     <li>根据 email 查询 sys_user：已存在则校验状态；不存在则自动注册并关联 GUEST 角色</li>
 *     <li>委托 {@link UserDetailsService} 加载完整用户信息（含角色）</li>
 *     <li>生成 access_token + refresh_token，保存授权记录</li>
 * </ol>
 * </p>
 *
 * <h3>自动注册逻辑（前端设计：新用户将自动注册）</h3>
 * <p>
 * 邮箱在 sys_user 表中不存在时，自动创建用户：
 * </p>
 * <ul>
 *     <li>用户名使用邮箱本身（因 email 已有 UNIQUE KEY，保证 username 唯一）</li>
 *     <li>密码为随机 UUID 加密值（邮箱验证码登录不使用密码，但字段非空）</li>
 *     <li>昵称取邮箱本地部分（@ 之前的内容）</li>
...
public class EmailCodeAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        // 3. 根据 email 查询 sys_user
        //    已存在:校验状态(禁用则拒绝)
        //    不存在:自动注册新用户并关联 GUEST 角色(前端设计:新用户将自动注册)
     ...
     * 用户名默认使用邮箱本身(因 email 已有 UNIQUE KEY,保证 username 唯一);
     * 极端情况下若 username 被占用(理论上不可能),追加 8 位随机后缀。
     * </p>
     *
     * <h3>字段填充规则</h3>
     * <ul>
     *     <li>username = email(若冲突则 email_&lt;random8&gt;)</li>
     *     <li>password = BCrypt(随机 UUID)(邮箱验证码登录不使用密码,但字段非空)</li>
     *     <li>nickname = 邮箱本地部分(@ 之前的内容)</li>
     *     <li>email = 用户邮箱</li>
    ...
    private SysUser autoRegisterUser(String email) {
        // 1. 生成用户名:默认使用邮箱(因 email UNIQUE,保证 username 唯一)
        String username = email;
        if (sysUserMapper.selectByUsername(username) != null) {
            // 极端情况兜底:username 被占用(理论上不可能,因 email 唯一)
            username = email + "_" + UUID.randomUUID().toString().substring(0, 8);
        }
        ...
        // 3. 昵称取邮箱本地部分(@ 之前的内容)
        String nickname = email.contains("@")
